...
async def ensure_browser():
    ...
    if browser_context is None:
        ...
        # 设置页面级别的超时时间
        main_page.set_default_timeout(150000)
...
async def login() -> str:
    ...
    await main_page.goto("https://www.xiaohongshu.com", timeout=150000)
...
#                         logging.info(f"获取标题时出错: {str(e)}")
#                         title = "未知标题"
#
#                     post_titles.append(title)
#             except Exception as e:
#                 logging.info(f"处理帖子卡片时出错: {str(e)}")
#
#         # 去重
#         unique_posts = []
#         seen_urls = set()
            ...
            try:
                ...
                if href and '/search_result/' in href:
                    ...
                    try:
                        ...
                        logging.info(f"获取标题失败: {e}")
                        post_titles.append("未知标题")
            except Exception as e:
                logging.info(f"处理卡片失败: {e}")
                continue

        # 限制返回数量
        post_links = post_links[:limit]
        post_titles = post_titles[:limit]

        # 构建结果
        result = {
            "keywords": keywords,
            "count": len(post_links),
            "notes": [{"title": t, "url": u} for t, u in zip(post_titles, post_links)]
        }

        return json.dumps(result, ensure_ascii=False)

    except Exception as e:
        logging.info(f"搜索笔记失败: {e}")
        return f"搜索失败: {str(e)}"
...
async def get_note_content(url: str) -> str:
    ...
    try:
        ...
        logging.info(f"处理后的URL: {processed_url}")
        ...
        try:
            ...
            logging.info(f"打印页面结构时出错: {str(e)}")
        ...
        try:
            ...
            logging.info(f"方法1获取标题出错: {str(e)}")
            post_content["标题"] = "未知标题"
        ...
        if post_content["标题"] == "未知标题":
            try:
                ...
                logging.info(f"方法2获取标题出错: {str(e)}")
        ...
        if post_content["标题"] == "未知标题":
            try:
                ...
                logging.info(f"方法3获取标题出错: {str(e)}")
        ...
        try:
            ...
            logging.info(f"方法1获取作者出错: {str(e)}")
            post_content["作者"] = "未知作者"
        ...
        if post_content["作者"] == "未知作者":
            try:
                ...
                logging.info(f"方法2获取作者出错: {str(e)}")
        ...
        if post_content["作者"] == "未知作者":
            try:
                ...
                logging.info(f"方法3获取作者出错: {str(e)}")
        ...
        try:
            ...
            logging.info(f"方法1获取发布时间出错: {str(e)}")
            post_content["发布时间"] = "未知"
        ...
        if post_content["发布时间"] == "未知":
            try:
                ...
                logging.info(f"方法2获取发布时间出错: {str(e)}")
        ...
        if post_content["发布时间"] == "未知":
            try:
                ...
                logging.info(f"方法3获取发布时间出错: {str(e)}")
        ...
        try:
            ...
            logging.info(f"方法1获取正文内容出错: {str(e)}")
            post_content["内容"] = "未能获取内容"
        ...
        if post_content["内容"] == "未能获取内容":
            try:
                ...
                logging.info(f"方法2获取正文内容出错: {str(e)}")
        ...
        if post_content["内容"] == "未能获取内容":
            try:
                ...
                logging.info(f"方法3获取正文内容出错: {str(e)}")
        ...
        if post_content["内容"] == "未能获取内容":
            try:
                ...
                logging.info(f"方法4获取正文内容出错: {str(e)}")
        ...
        if post_content["内容"] == "未能获取内容":
            try:
                ...
                logging.info(f"方法5获取正文内容出错: {str(e)}")
...
async def get_note_comments(url: str) -> str:
    ...
    try:
        # 处理URL
        processed_url = process_url(url)
        logging.info(f"处理后的评论URL: {processed_url}")
        ...
        try:
            ...
            logging.info(f"创建评论区定位器时出错: {str(e)}")
            # 继续执行，不阻断程序
        ...
        for locator in comment_section_locators:
            try:
                ...
                logging.info(f"滚动到评论区时出错: {str(e)}")
                continue
            ...
            try:
                ...
                for selector in more_comment_selectors:
                    try:
                        ...
                        logging.info(f"点击查看更多按钮时出错: {str(e)}")
                        continue
            except Exception as e:
                logging.info(f"滚动页面加载更多评论时出错: {str(e)}")
                pass
        ...
        for selector in comment_selectors:
            try:
                            ...
                            try:
                                ...
                                if username == "未知用户":
                                    try:
                                        ...
                                        logging.info(f"通过用户链接获取用户名出错: {str(e)}")
                ...
                logging.info(f"处理评论选择器出错: {str(e)}")
                continue
    ...
    try:
        ...
        for selector in comment_area_selectors:
            try:
                ...
                logging.info(f"定位评论区域时出错: {str(e)}")
                continue
        ...
        for selector in input_selectors:
            try:
                ...
                logging.info(f"定位评论输入框时出错: {str(e)}")
                continue
            ...
            if js_result:
                ...
                for selector in input_selectors:
                    try:
                        ...
                        logging.info(f"尝试再次查找输入框时出错: {str(e)}")
                        continue
        ...
        if not send_success:
            try:
                ...
                logging.info(f"使用Enter键发送出错: {str(e)}")
        ...
        if not send_success:
            try:
                ...
                logging.info(f"使用JavaScript点击发送按钮出错: {str(e)}")
